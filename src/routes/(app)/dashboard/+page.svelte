...
<div class="space-y-8 animate-in fade-in duration-200">
		...
		<div class="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-8">
			<div class="space-y-4 max-w-2xl">
				...
				<div class="flex items-center gap-2 flex-wrap">
					...
					<span class="px-3 py-1 rounded-full bg-white/80 border border-black/10 text-black text-[11px] font-mono font-bold flex items-center gap-1.5">
						...
						<span>On Track (30-day target)</span>
					</span>
					<span class="px-3 py-1 rounded-full bg-white/80 border border-black/10 text-black text-[11px] font-mono font-bold flex items-center gap-1.5">
						<Flame class="w-3 h-3 text-orange-600" />
				...
				<div>
					<h1 class="text-3xl sm:text-4xl lg:text-5xl font-bold tracking-tight text-black font-display leading-[1.08]">
						{goal ? goal.title : 'Graph Algorithms for Coding Interviews'}
					</h1>
					<p class="text-sm sm:text-base text-black/80 font-light mt-2 leading-relaxed max-w-xl">
						Target: <strong>{goal?.targetOutcome || 'Master graph algorithms and problem patterns'}</strong>. 
						Allocated budget is <strong>{goal?.dailyMinutesBudget || 45} min/day</strong> across {goal?.studyDaysPerWeek || 5} study days per week.
					</p>
				</div>
				...
				<div class="space-y-1.5 pt-2 max-w-md">
					...
					<div class="w-full h-3 rounded-full bg-black/15 overflow-hidden p-0.5">
						<div class="h-full bg-black rounded-full transition-all duration-500" style="width: 78%"></div>
					</div>
				</div>
			</div>
			...
			<div class="flex flex-col sm:flex-row lg:flex-col gap-3 shrink-0">
				...
				>
					...
					<span>Start Today's Session ({goal?.dailyMinutesBudget || 45}m)</span>
	...
	<!-- 4. 4 HIGH-FREQUENCY CORE MODULES (DESIGN.md Pastel Sticky-Note Tiles) -->
	<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
		...
		>
			<div class="space-y-2">
				...
				<p class="text-xs text-black/70 leading-relaxed font-light">
					Diagnostic probes with 60% priority on weak areas like Dijkstra.
				</p>
			</div>
			<span class="text-xs font-mono font-bold text-black mt-3 flex items-center gap-1">
				<span>Take Quiz</span>
		...
		<!-- Socratic Coach (Coral #f3c9b6) -->
		<a
			href={`/goals/${goal?.id}/teach-back`}
			class="rounded-3xl bg-[#f3c9b6] p-6 border border-black/10 hover:shadow-md transition-all group flex flex-col justify-between text-black min-h-[170px]"
		>
			<div class="space-y-2">
				<div class="flex items-center justify-between">
					<MessageSquare class="w-6 h-6 text-black" />
					<span class="text-[10px] font-mono font-bold text-black/60 uppercase">Socratic Coach</span>
				</div>
				<h3 class="text-base font-bold font-display text-black group-hover:underline">Teach Me Mode</h3>
				<p class="text-xs text-black/70 leading-relaxed font-light">
					Explain algorithms in your own words; Ada grades depth and invariants.
				</p>
			</div>
			<span class="text-xs font-mono font-bold text-black mt-3 flex items-center gap-1">
				<span>Start Teach-Back</span>
				<ArrowRight class="w-3.5 h-3.5 group-hover:translate-x-1 transition-transform" />
			</span>
		</a>
	</div>

	<!-- 5. SPLIT SECTION: "WHAT SHOULD I DO RIGHT NOW?" (LEFT) + FEASIBILITY ENGINE (RIGHT) -->
	<div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
		<!-- Left (7 cols): Today's 4-Part Daily Plan -->
		<div class="lg:col-span-7 bg-white dark:bg-[#121316] rounded-3xl p-6 sm:p-8 border border-[#e6e6e6] dark:border-white/[0.08] shadow-sm space-y-6 text-black dark:text-zinc-100 transition-colors">
			<!-- Header -->
			<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-[#e6e6e6] dark:border-zinc-800">
				<div>
					<span class="text-[10px] font-mono font-bold uppercase tracking-wider text-black/50 dark:text-zinc-400">Daily Itinerary</span>
					<h2 class="text-xl font-bold text-black dark:text-zinc-100 font-display">What should I do right now?</h2>
				</div>
		...
		<!-- Right (5 cols): Feasibility Reality Check (DESIGN.md Lime Section) -->
		<div class="lg:col-span-5 bg-[#dceeb1] rounded-3xl p-6 sm:p-8 border border-black/10 shadow-sm space-y-6 text-black">
			...
			<p class="text-xs text-black/80 leading-relaxed font-light">
				{goal?.feasibility?.summary || 'Your goal is achievable with targeted interview practice. Consider one of the strategic alternatives below to stay comfortably on pace.'}
			</p>

			<!-- Strategic Alternatives (Options A, B, C, D) -->
